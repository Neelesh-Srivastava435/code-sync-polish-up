...
class MemberController extends Controller
{
    ...
    public function index()
    {
        $members = Member::with([
            'batches' => function ($query) {
                ...
                $query->where('batches.status', 'active') // Also qualify status if it's ambiguous, though less likely for 'status'
                      ->select('batches.id', 'batches.name', 'batches.program_id');
            },
            'batches.program:id,name'
        ])
        ->select('members.id', 'members.name', 'members.email', 'members.mobile', 'members.status') // Qualify member columns too for clarity
    ...
    public function show($id)
    {
        ...
        ->select('members.id', 'members.name', 'members.email', 'members.mobile', 'members.status') // Qualify member columns
        ->find($id);
