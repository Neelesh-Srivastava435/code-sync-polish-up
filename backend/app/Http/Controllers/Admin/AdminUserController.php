...
class AdminUserController extends Controller
{
    ...
    {
        try {
            $users = AdminUser::with(['role', 'permission'])->orderBy('created_at', 'desc') ->get()->map(function ($user) {
    ...
    public function updateAuthenticatedUserProfile(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            // Note: email and phone validation should allow the current user's existing values
        ...
        $this->logUserAction(
            ...
            $user,
            "Authenticated user updated their profile details.",
            $originalData,
            $user->only(['first_name', 'last_name', 'email', 'phone', 'profile_image']) // Include profile_image in log
        );
