...
class BulkImportController extends Controller
{
    public function importUsers(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'users.*.email' => 'required|email|distinct|unique:admin_users,email', // Ensure email is unique in the request and in the DB
            'users.*.phone' => 'nullable|string|max:20|distinct|unique:admin_users,phone',
            'users.*.role' => 'required|string|in:Admin,Account Manager,Facility Manager', // Ensure role is valid
            'users.*.status' => 'required|string|in:active,pending,inactive', // Frontend sends lowercase
            ...
            'users.*.address' => 'nullable|string|max:65535', // Max length for TEXT type
