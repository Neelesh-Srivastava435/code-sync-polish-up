...
const AddVenueSpots: React.FC<AddVenueSpotsProps> = ({
  ...
  const handleConfirmDeleteSpot = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to remove the spot.",
        variant: "destructive"
      });
    } finally {
      setSpotToDelete(null); // Reset spot to delete
  ...
  const validateForm = (): boolean => {
    ...
    // Validate spot name
    if (!currentSpotData.spot_name.trim()) {
      errors.spot_name = "Spot name is required";
      isValid = false;
    }
    ...
    if (!currentSpotData.operative_days || currentSpotData.operative_days.length === 0) {
      errors.operative_days = "At least one operative day must be selected";
      isValid = false;
    }

    // Validate times
    if (!currentSpotData.start_time) {
      errors.start_time = "Start time is required";
      isValid = false;
    }
    if (!currentSpotData.end_time) {
      errors.end_time = "End time is required";
      isValid = false;
    }
    if (currentSpotData.start_time && currentSpotData.end_time && 
        currentSpotData.start_time >= currentSpotData.end_time) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to save the spot.",
        variant: "destructive"
      });
    }
  ...
  const handleInputChange = (field: keyof typeof currentSpotData) => (
    ...
    setCurrentSpotData(prev => ({ ...prev, [field]: value }));
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={validation.spot_name ? "border-red-500" : ""}
                  />
                  {validation.spot_name && (
                    <p className="text-xs text-red-500">{validation.spot_name}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className={validation.area ? "border-red-500" : ""}
                    />
                    {validation.area && (
                      <p className="text-xs text-red-500">{validation.area}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className={validation.capacity ? "border-red-500" : ""}
                    />
                    {validation.capacity && (
                      <p className="text-xs text-red-500">{validation.capacity}</p>
                    )}
                  </div>
                </div>
