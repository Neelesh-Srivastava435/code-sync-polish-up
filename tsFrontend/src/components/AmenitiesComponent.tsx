...
import { handleApiError, isValidationError } from '@/utils/errorHandling';
              ...
              <DialogContent>
                ...
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <div className={`${isMobile ? 'col-span-4' : 'col-span-3'}`}>
                      <div className={`flex flex-wrap gap-2 p-2 border rounded-md ${validationErrors.icon ? 'border-red-500' : ''}`}>
                        {iconOptions.map((icon) => (
                          <div
                            key={icon}
                            className={`cursor-pointer p-2 rounded-md hover:bg-gray-100 ${
                              newAmenity.icon === icon ? 'bg-blue-100 border border-blue-300' : ''
                            }`}
                            onClick={() => {
                              setNewAmenity({...newAmenity, icon});
                              setValidationErrors({...validationErrors, icon: undefined});
                            }}
                          >
                            {icon}
                          </div>
