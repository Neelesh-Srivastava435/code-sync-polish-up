...
import { Edit, AlertTriangle, MapPin, Images } from 'lucide-react';
...
const SessionTable: React.FC<SessionTableProps> = ({ 
  ...
  const getSessionNumber = (session: Session): number => {
    return session.sessionNumber ?? parseInt(session.id) % 100;
  };
          ...
          {sessions.length === 0 ? (
            ...
            sessions.map((session) => (
              <TableRow key={session.id}>
                <TableCell>Session {getSessionNumber(session)}</TableCell>
